...
def calcular_ir(valor_bruto, dependentes):
    """Calcula o Imposto de Renda sobre o valor bruto."""
    deducao_dependentes = dependentes * 189.59  # Deduções por dependente
    
    if valor_bruto - deducao_dependentes <= 1903.98:
        return 0
    elif valor_bruto - deducao_dependentes <= 2826.65:
        return (valor_bruto - deducao_dependentes) * 0.075 - 142.80
    elif valor_bruto - deducao_dependentes <= 3751.05:
        return (valor_bruto - deducao_dependentes) * 0.15 - 354.80
    elif valor_bruto - deducao_dependentes <= 4664.68:
        return (valor_bruto - deducao_dependentes) * 0.225 - 636.13
    else:
        return (valor_bruto - deducao_dependentes) * 0.275 - 869.36
...
if ferias_vencidas == "Sim":
    num_ferias_vencidas = st.number_input(
        "Quantas férias vencidas houveram no ano anterior?",
        min_value=0,
        max_value=5,
        value=0
    )
...
if st.button("Calcular Verbas Rescisórias", type="primary"):
    ...
    if meses_trabalhados <= 0:
        ...
        valor_ferias_total = valor_ferias_prop_base + valor_terco_constitucional + (num_ferias_vencidas * salario_base)  # Acrescenta férias vencidas
        valor_aviso_previo = calcular_aviso_previo(data_admissao, data_demissao, salario_base, motivo_rescisao)
        ...
        ir = calcular_ir(total_bruto_rescisao, dependentes)
